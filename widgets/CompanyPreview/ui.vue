<template>
  <div class="h-dvh border-b border-b-black pt-[3.2rem] overflow-hidden mb:h-[calc(100dvh-6.2rem)]">
    <VideoUi
      class="absolute top-0 left-0 right-0 w-full h-dvh max-h-dvh object-cover"
      :src="getDomainLink('/video/video-logo.mp4')"
      :poster="getDomainLink('/images/video-logo-poster.png')"
    />

    <div
      class="absolute z-10 top-0 left-0 right-0 h-dvh max-h-dvh bg-black/[68%]"
    />
